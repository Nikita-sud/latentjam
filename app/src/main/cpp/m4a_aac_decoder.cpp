 ...
 * m4a_aac_decoder.cpp is part of LatentJam.
 *
...
bool decode_m4a_aac(const uint8_t *data, size_t size, int64_t start_us,
    ...
    MP4D_demux_t demux { };
    if (!MP4D_open(&demux, mmap_read_callback, &ctx,
    ...
    for (unsigned i = 0; i < demux.track_count; ++i) {
        if (demux.track[i].handler_type == MP4D_HANDLER_TYPE_SOUN
                && demux.track[i].dsi != nullptr && demux.track[i].dsi_bytes > 0
    ...
    // cases.
    uint64_t start_ts = static_cast<uint64_t>(start_us) * tr.timescale
            / 1000000;
    ...
    out.mono = static_cast<float*>(std::malloc(
            target_per_channel * sizeof(float)));
    if (!out.mono) {
        aacDecoder_Close(hAac);
        MP4D_close(&demux);
        return false;
    ...
    for (unsigned i = start_frame;
        ...
        MP4D_file_offset_t off = MP4D_frame_offset(&demux, track_idx, i,
                &frame_bytes, &ts, &dur);
        if (frame_bytes == 0 || off + frame_bytes > size)
            break;

        UCHAR *inbuf[1] = { const_cast<UCHAR*>(data + off) };
        UINT in_size[1] = { frame_bytes };
        UINT bytes_valid = frame_bytes;
        if (aacDecoder_Fill(hAac, inbuf, in_size, &bytes_valid) != AAC_DEC_OK)
            continue;

        AAC_DECODER_ERROR e = aacDecoder_DecodeFrame(hAac, pcm_buf,
                PCM_BUF_SAMPLES, 0);
        if (e == AAC_DEC_NOT_ENOUGH_BITS)
            continue;
        if (e != AAC_DEC_OK)
            continue;  // skip one frame on decode errors

        CStreamInfo *si = aacDecoder_GetStreamInfo(hAac);
        if (!si || si->frameSize <= 0)
            continue;
    ...
    LOGI("m4a OK sr=%d ch=%d got=%zu (from frame %u/%u)", actual_sr, channels,
            out_pos, start_frame, tr.sample_count);
    return true;
}
