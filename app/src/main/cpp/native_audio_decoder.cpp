...
void free_decoded(Decoded& d) {
    if (d.mono) { std::free(d.mono); d.mono = nullptr; }
    d.samples = 0;
}
...
bool decode_mp3(const uint8_t* data, size_t size, int64_t start_us,
    ...
    if (size <= SAFETY_PAD + 1024) return false;
    ...
    uint64_t start_sample_interleaved =
        static_cast<uint64_t>(start_us * sample_rate / 1000000) *
        static_cast<uint64_t>(channels);
    if (total_samples > 0 && start_sample_interleaved >= total_samples) {
        start_sample_interleaved = total_samples > static_cast<uint64_t>(channels)
    ...
    size_t target_interleaved = target_per_channel * static_cast<size_t>(channels);

    float* interleaved = static_cast<float*>(std::malloc(target_interleaved * sizeof(float)));
    if (!interleaved) { mp3dec_ex_close(&dec); return false; }

    size_t got = mp3dec_ex_read(&dec, interleaved, target_interleaved);
    mp3dec_ex_close(&dec);

    if (got == 0) { std::free(interleaved); return false; }
    ...
    for (size_t i = 0; i < dbg_n; ++i) {
        if (interleaved[i] < dbg_min) dbg_min = interleaved[i];
        if (interleaved[i] > dbg_max) dbg_max = interleaved[i];
    }
    LOGI("mp3 raw scan: min=%.4f max=%.4f (over %zu samples)", dbg_min, dbg_max, dbg_n);

    out.mono = static_cast<float*>(std::malloc(got_per_channel * sizeof(float)));
...
bool decode_opus(const uint8_t* data, size_t size, int64_t start_us,
    ...
    ogg_int64_t start_sample = static_cast<ogg_int64_t>(start_us * OPUS_SR / 1000000);
    if (op_pcm_seek(of, start_sample) != 0) op_pcm_seek(of, 0);

    size_t target_per_channel = static_cast<size_t>(duration_us * OPUS_SR / 1000000);
    if (target_per_channel == 0) { op_free(of); return false; }

    float* interleaved = static_cast<float*>(
        std::malloc(target_per_channel * static_cast<size_t>(channels) * sizeof(float)));
    if (!interleaved) { op_free(of); return false; }
    ...
    while (produced < target_per_channel) {
        ...
        int n = op_read_float(
            ...
            nullptr);
        if (n < 0) { std::free(interleaved); op_free(of); return false; }
        if (n == 0) break;
        produced += static_cast<size_t>(n);
    }
    op_free(of);

    if (produced == 0) { std::free(interleaved); return false; }
...
bool decode_flac(const uint8_t* data, size_t size, int64_t start_us,
    ...
    if (channels < 1 || sample_rate <= 0) { drflac_close(f); return false; }

    drflac_uint64 start_frame =
        static_cast<drflac_uint64>(start_us) * sample_rate / 1000000;
    ...
    drflac_uint64 target_frames =
        static_cast<drflac_uint64>(duration_us) * sample_rate / 1000000;
    if (target_frames == 0) { drflac_close(f); return false; }

    float* interleaved = static_cast<float*>(
        std::malloc(target_frames * static_cast<size_t>(channels) * sizeof(float)));
    if (!interleaved) { drflac_close(f); return false; }

    drflac_uint64 got = drflac_read_pcm_frames_f32(f, target_frames, interleaved);
    drflac_close(f);
...
bool decode_wav(const uint8_t* data, size_t size, int64_t start_us,
    ...
    if (channels < 1 || sample_rate <= 0) { drwav_uninit(&wav); return false; }

    drwav_uint64 start_frame =
        static_cast<drwav_uint64>(start_us) * sample_rate / 1000000;
    if (total_frames > 0 && start_frame >= total_frames) {
        start_frame = total_frames > 1 ? total_frames - 1 : 0;
    }
    ...
    drwav_uint64 target_frames =
        static_cast<drwav_uint64>(duration_us) * sample_rate / 1000000;
    if (target_frames == 0) { drwav_uninit(&wav); return false; }

    float* interleaved = static_cast<float*>(
        std::malloc(target_frames * static_cast<size_t>(channels) * sizeof(float)));
    if (!interleaved) { drwav_uninit(&wav); return false; }

    drwav_uint64 got = drwav_read_pcm_frames_f32(&wav, target_frames, interleaved);
    drwav_uninit(&wav);
...
bool decode_vorbis(const uint8_t* data, size_t size, int64_t start_us,
    ...
    if (channels < 1 || sample_rate <= 0) { stb_vorbis_close(v); return false; }

    unsigned int start_sample =
        static_cast<unsigned int>(start_us * sample_rate / 1000000);
    ...
    size_t target_per_channel =
        static_cast<size_t>(duration_us * sample_rate / 1000000);
    if (target_per_channel == 0) { stb_vorbis_close(v); return false; }

    float* interleaved = static_cast<float*>(
        std::malloc(target_per_channel * static_cast<size_t>(channels) * sizeof(float)));
    if (!interleaved) { stb_vorbis_close(v); return false; }

    size_t produced = 0;
    while (produced < target_per_channel) {
        int remaining_per_channel =
            static_cast<int>(target_per_channel - produced);
...
size_t id3v2_skip(const uint8_t* data, size_t size) {
    ...
    uint32_t sz = (uint32_t(data[6]) << 21) |
                  (uint32_t(data[7]) << 14) |
                  (uint32_t(data[8]) <<  7) |
                  (uint32_t(data[9]));
...
Format ogg_codec(const uint8_t* data, size_t size) {
    ...
    if (n < 16) return FMT_OPUS;  // not enough to tell; existing libopusfile fail-mode handles it
...
Format sniff(const uint8_t* data, size_t size) {
    ...
    if (data[4] == 'f' && data[5] == 't' && data[6] == 'y' && data[7] == 'p') {
        ...
        auto is = [&](const char a, const char b, const char c, const char d) {
            return brand[0] == a && brand[1] == b && brand[2] == c && brand[3] == d;
        };
        if (is('M','4','A',' ') || is('M','4','B',' ') || is('M','4','P',' ') ||
            is('m','p','4','2') || is('m','p','4','1') ||
            is('i','s','o','m') || is('i','s','o','2') || is('i','s','o','5') ||
            is('d','a','s','h') || is('f','4','a',' ')) {
            return FMT_M4A;
        }
    ...
    if (data[0] == 'I' && data[1] == 'D' && data[2] == '3') {
        ...
        if (skip > 0 && skip + 4 <= size) {
            ...
            if (p[0] == 0xFF && (p[1] & 0xE0) == 0xE0) return FMT_MP3;
        }
