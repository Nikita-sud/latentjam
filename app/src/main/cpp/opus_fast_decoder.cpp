 ...
 * opus_fast_decoder.cpp is part of LatentJam.
 *
...
bool build_index(const std::uint8_t *data, std::size_t size,
    ...
    std::size_t off = 0;
    while (off + 27 <= size) {
        if (data[off] != 'O' || data[off + 1] != 'g' || data[off + 2] != 'g'
                || data[off + 3] != 'S') {
...
bool parse_opushead(const std::uint8_t *data, const PageInfo &page,
    ...
    if (page.total_size - page.header_size < 19)
        return false;
...
std::size_t page_for_granule(const std::vector<PageInfo> &pages,
    ...
    while (lo < hi) {
        ...
        if (g != UINT64_MAX && g < target_granule)
            lo = mid + 1;
        else
            hi = mid;
    }
...
bool decode_opus_fast(const std::uint8_t *data, std::size_t size,
    ...
    std::vector<PageInfo> pages;
    pages.reserve(1024);
    if (!build_index(data, size, pages))
        return false;
    const int64_t t_idx = monotonic_ns();

    OpusHead head { };
    if (!parse_opushead(data, pages[0], head))
        return false;
    ...
    const std::uint64_t start_granule = static_cast<std::uint64_t>(start_us)
            * OUT_SR / 1000000 + head.pre_skip;
    const std::size_t target_per_channel = static_cast<std::size_t>(duration_us)
            * OUT_SR / 1000000;
    ...
    float *interleaved = static_cast<float*>(std::malloc(
            target_per_channel * channels * sizeof(float)));
    if (!interleaved) {
        opus_decoder_destroy(dec);
        return false;
    }
    ...
    for (std::size_t pi = first_page;
        ...
        const std::uint8_t *seg_data = data + page.byte_offset
                + page.header_size;
        ...
        for (std::uint8_t i = 0; i < n_segs; ++i) {
            ...
            if (s < 255) {
                ...
                if (!pkt.empty()) {
                    int n = opus_decode_float(dec, pkt.data(),
                            static_cast<opus_int32>(pkt.size()), frame_buf,
                            MAX_FRAME, /*decode_fec=*/0);
                    pkt.clear();
                    if (n <= 0)
                        continue;
                    ...
                    std::memcpy(interleaved + produced * channels,
                            frame_buf + skip * channels,
                            emit * channels * sizeof(float));
                    produced += emit;
                    any_decoded = true;
                    if (produced >= target_per_channel)
                        break;
                }
            }
        }
    ...
    LOGI(
            "opus(fast) OK ch=%d pages=%zu got=%zu | idx=%.2fms open=%.2fms decode+downmix=%.2fms total=%.2fms",
            channels, pages.size(), produced, (t_idx - t0) / 1e6,
            (t_dec_open - t_idx) / 1e6, (t_end - t_dec_open) / 1e6,
            (t_end - t0) / 1e6);
    return true;
}
