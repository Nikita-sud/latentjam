 ...
 * + temperature-sampled head. No learned scorer involved — `scoring_v1.pt` produced
 * uncalibrated scores on real personal libraries (e.g. SpongeBob production music
 * surfaced for rap seeds), so we lean on the audio encoder's neighbors and shape them
 * with three cheap rules:
 *
 *   1. **Centroid seed** — average the embeddings of the current track + last K=4
 *      completed plays. One outlier in the recent window can't pull the chain off
 *      course.
 *   2. **Artist cooldown** — drop any candidate whose artist appears in the recent
 *      window. Without this, nearest-neighbor lookups collapse to 4–5 tracks from
 *      one artist in a row.
 *   3. **BPM cooldown** — drop candidates whose tagged BPM differs from the current
 *      track's by more than 15 %. MusicNN embeddings are timbre-heavy and don't
 *      always encode tempo, so without this rule SMART occasionally yo-yos
 *      between 140 BPM phonk and 90 BPM chillout because the model thinks they
 *      "sound similar". Filter is a no-op when either track lacks a TBPM/BPM tag.
 *   4. **Deterministic head** — top TEMP_TOP_K candidates by BPM-penalty-adjusted
 *      cosine. **Was originally temperature-sampled at T=0.3** but the 150-decision
 *      ablation showed that the inter-pick cosine gaps in our library (~0.01-0.03)
 *      were dwarfed by Gumbel(0,1) noise (std 1.28), making every activation reshuffle
 *      essentially randomly (0/50 seeds stable across 3 runs, Jaccard 0.17). Same
 *      seed now produces the same queue.
 *
 * Triggering on `onIndexMoved` is the only reliable signal: `onProgressionChanged` fires
 * only on play/pause/seek and is silent during steady-state playback. Queuing at
 * index-move time is functionally equivalent — `playNext()` inserts at `index + 1`, and
 * when the current track ends ExoPlayer advances to that slot.
 *
...
class RecommendationEngine @Inject constructor(
    ...
    private fun restorePreSmartUpcoming() {
        ...
        scope.launch {
            withContext(Dispatchers.Main) {
                playbackStateManager.replaceUpcoming(songs)
            }
        }
    }

    private fun maybeInject(reason: String) {
        if (playbackStateManager.shuffleMode !=
            io.github.nikitasud.latentjam.playback.state.ShuffleMode.SMART) return
        if (queueIsSmart) return
    ...
    /**
     * Toggles for the BPM / energy filters. Used by the ablation sweep to attribute
     * pick-quality changes to each filter individually instead of looking at both
     * stacked. **Production uses [PRODUCTION_FILTER_CONFIG], not BOTH** — the ablation
     * (15 seeds × 4 configs on a 700-track library) showed bpm-only at 87 % "on vibe"
     * vs both-filters at 60 %. The RMS-based energy signal hurts more than it helps:
     * tracks with similar loudness aren't necessarily similar tracks, so a multiplicative
     * energy penalty drags weak picks into the head. Revisit when we have a better
     * energy signal (e.g. learned from feedback events).
     */
    ...
    private suspend fun runPipeline(
        ...
        if (embeddedCount < MIN_LIBRARY_FOR_PREDICTOR) {
            Timber.v("cold-start: %d embedded < %d, skip", embeddedCount, MIN_LIBRARY_FOR_PREDICTOR)
            return@withContext
        }
        ...
        val seedUids = LinkedHashSet<Music.UID>().apply {
            add(currentSong.uid)
            recentCompleted.forEach { add(it.uid) }
        }.toList().take(CENTROID_SIZE)

        val seedEmbeddings = ArrayList<FloatArray>(seedUids.size)
        for (uid in seedUids) {
            val emb = trackEmbeddingDao.get(uid, version)?.embedding
                ?.let { bytesToFloats(it, Retrieval.EMBEDDING_DIM) }
        ...
        val sorted = if (predictorState != null) {
        ...
        val cooldownArtistIds = HashSet<Music.UID>().apply {
        ...
        val seedTempo = if (seedIdx >= 0) snapshot.tempos[seedIdx].takeIf { !it.isNaN() } else null
        val seedEnergy = if (seedIdx >= 0) snapshot.energies[seedIdx].takeIf { !it.isNaN() } else null
        ...
        for (hit in sorted) {
            ...
            val bpmPenalty = if (filterConfig.bpmEnabled && seedTempo != null && candidateTempo != null) {
            ...
            val energyPenalty = if (filterConfig.energyEnabled && seedEnergy != null && candidateEnergy != null) {
            ...
            val passesDedup = poolSongs.size >= POOL_SIZE ||
        ...
        poolSongs.clear(); poolSongs.addAll(poolSongsSorted)
        poolScores.clear(); poolScores.addAll(poolScoresSorted)
        poolUidIndices.clear(); poolUidIndices.addAll(poolUidIndicesSorted)
        ...
        val scorerScores: FloatArray? = if (predictorState != null) {
            try {
                ...
                val candidates = Array(n) { i ->
                    if (i < poolUidIndices.size) {
                        ...
                        FloatArray(Retrieval.EMBEDDING_DIM) { d -> snapshot.rawFlat[base + d] }
                    } else {
        ...
        val poolScorerScores: FloatArray? = scorerScores?.let { logits ->
        ...
        if (poolScorerScores != null) {
            ...
            poolSongs.clear(); poolSongs.addAll(s2)
            poolScores.clear(); poolScores.addAll(c2)
            poolUidIndices.clear(); poolUidIndices.addAll(u2)
        ...
        val sampledHeadWithScores = (0 until headTake).map { i ->
        ...
        val sampledHeadUids = HashSet<Music.UID>(sampledHead.size).apply {
            sampledHead.forEach { add(it.uid) }
        }
        ...
        smartSessionLog.recordDecision(
            ...
            seedArtists = currentSong.artists.joinToString(", ") { artist ->
            ...
            tailSize = tail.size,
            headPicks = sampledHeadWithScores.mapIndexed { i, (song, _, _) ->
                ...
                val songTempo = if (idx >= 0) snapshot.tempos[idx].takeIf { !it.isNaN() } else null
                val scorerLogit = poolScorerScores?.getOrNull(i)
                    ?.takeIf { it != Float.NEGATIVE_INFINITY }
                SmartSessionLog.Pick(
                    uid = song.uid.toString(),
                    name = song.name.raw.takeIf { it.isNotBlank() },
                    artists = song.artists.joinToString(", ") { artist ->
                        (artist.name as? org.oxycblt.musikr.tag.Name.Known)?.raw
                            ?: artist.name.toString()
                    }.takeIf { it.isNotBlank() },
                    bpm = songTempo,
        ...
        withContext(Dispatchers.Main) {
            playbackStateManager.replaceQueueAroundCurrent(picks)
        }
    }

    /**
     * Comprehensive dry-run sweep: pick [SWEEP_SIZE] random library seeds and run each
     * through [SWEEP_REPEATS] independent passes under the production filter config
     * (BPM-only). Same code path as live SMART so the picks emitted here mirror what
     * the user actually sees in the app for those seeds.
     *
     * Why N repeats per seed: temperature sampling uses fresh Gumbel(0,1) noise on
     * every activation. For an encoder-confident seed (anime, eurodance, classic rock)
     * the pool's top scores are widely separated and the noise can't reorder them —
     * three runs produce nearly the same head. For an encoder-uncertain seed (hip-hop
     * in this library, anything cross-cluster) the pool scores are flat and the noise
     * dominates, producing different heads each run. Comparing the 3 runs per seed
     * surfaces *which* seeds are genuinely well-served by the encoder vs which are
     * being shuffled by noise.
     *
    ...
    private fun launchDryRunSweep() {
        scope.launch {
            ...
            while (System.currentTimeMillis() < deadlineMs) {
                ...
                if (lib != null && trackEmbeddingDao.count(mlSettings.embeddingVersion)
                        >= MIN_LIBRARY_FOR_PREDICTOR) break
            ...
            for (run in runs) {
                for (seed in sample) {
                    try {
                        ...
                        Timber.w(e, "dry-run sweep: pipeline failed for %s under %s",
                            seed.uid, run.label)
                    }
                }
            }
    ...
    /**
     * Multiplicative score penalty as a function of the tempo gap between seed and
     * candidate. Octave-folded: ratios above 1.5 get divided by 2 first, so tracks
     * one octave apart (60 ↔ 120) match perfectly. Falls off linearly with the gap
     * and floors at [BPM_PENALTY_FLOOR] so even maximum-mismatch candidates remain
     * eligible (no eliminations — just re-ranking).
     *
     *   ratio 1.00 → 1.0    (exact match)
     *   ratio 1.10 → 0.8    (10 % off)
     *   ratio 1.25 → 0.5    (25 % off)
     *   ratio 1.50 → 0.3    (50 % off — floor)
     */
    ...
    /**
     * Title normalization for within-head dedup. Strips parenthetical version markers,
     * common suffixes ("- radio edit", "feat. X", "(slowed + reverb)"), and trailing
     * digits so different mixes / sequels of the same track collapse. Aggressive on
     * purpose — false-positive merges are OK at the dedup level because we still have
     * 50 candidates in the pool; false negatives are what we're fixing.
     */
    ...
    companion object {
        ...
        /**
         * Top-N cosine-sorted survivors kept as the head pool. Pinned at the
         * scorer's static input size so we can feed the full pool through
         * `PredictorRuntime.score` without zero-padding (pad rows still go in
         * to fill the (1, 100, D) tensor when cooldown leaves < 100 candidates,
         * but their scores are masked to -inf afterwards).
         */
        ...
        /**
         * BPM filter tolerance — candidates whose tagged BPM falls outside
         * `[curr × (1 - T), curr × (1 + T)]` are dropped. Only fires when both the
         * current track AND the candidate have a BPM tag; missing tags pass
         * through so the filter degrades to a no-op on un-tagged libraries.
         */
        /**
         * Multiplier on the energy distance penalty applied to cosine scores. With weight
         * `W`, a candidate at full energy distance (Δ=1.0) gets its score reduced by `W`.
         * Set to 0.5 so the strongest energy-distant candidates still survive but the head
         * favors energy-matched picks. 0 disables the penalty.
         */
        private const val ENERGY_PENALTY = 0.5f

        /**
         * Floor for the BPM penalty multiplier — even the worst tempo mismatch never zeroes
         * a candidate. The reviewer's recipe: 0.3 keeps badly-mismatched candidates "still
         * in the running, just heavily penalized" so the cosine signal can still win on a
        ...
        /**
         * Repeats per seed in the sweep. Each repeat re-rolls the temperature sampler's
         * Gumbel noise, so comparing the 3 picks per seed shows whether the encoder is
         * confident (3 nearly-identical heads) or guessing (3 wildly different heads).
         */
