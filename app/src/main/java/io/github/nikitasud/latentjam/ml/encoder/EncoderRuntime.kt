 ...
 * EncoderRuntime.kt is part of LatentJam.
 *
...
/**
 * Encoder runtime backed by **ONNX Runtime + QNN HTP** (Qualcomm Hexagon NPU) running the
 * LAION-CLAP HTSAT audio tower (27.5 M params, Apache-2.0, 512-d L2-normalized embeddings).
 *
 * **Input contract**: raw mono float32 PCM at 48 kHz with shape `(B, 480_000)` — ten seconds per
 * window. The mel-spectrogram frontend (STFT via conv1d, Hann window, n_fft=1024, hop=480, 64-mel
 * htk filterbank, log-dB, expand to the 4-chunk fusion bundle) is baked into the ONNX graph.
 * Output: `(B, 512)` L2-normalized embedding.
 *
 * **Backends, in cascade order**:
 * 1. `QNN_HTP` — Hexagon V75 NPU. With `enable_htp_fp16_precision`, ORT runtime-converts FP32
 *    weights to fp16 and offloads to HTP. Measured ~58 ms warm per window. HTP-vs-CPU cosine parity
 *    ≥ 0.999924 on 10 seeded inputs.
 * 2. `CPU` — ORT's CPU EP. Fallback for non-Qualcomm devices. ~205 ms warm per window.
 *
 * The HTSAT graph uses a rank-5 window-partition rewrite — upstream HF's rank-6 reshape triggers
 * QNN graphAddNode op-error 6007. See `latentjam-research/scripts/export_clap_audio_onnx.py` for
 * the export.
 *
 * The HTP path requires three non-obvious bits of plumbing wired up around this class:
 * - `<uses-native-library android:name="libcdsprpc.so" required=false/>` in the manifest, so
 *   FastRPC to the Hexagon DSP is dlopen-able by a non- system app on Android 13+.
 * - `useLegacyPackaging = true` on `jniLibs` in `build.gradle`, so AGP extracts
 *   `libQnnHtpV??Skel.so` to disk; the DSP-side dynamic loader cannot read libs embedded inside an
 *   unextracted APK.
 * - `com.microsoft.onnxruntime:onnxruntime-android-qnn` as the gradle dep — the `-qnn` flavor of
 *   the AAR transitively pulls `com.qualcomm.qti:qnn-runtime`, which ships the per-Hexagon-arch
 *   `libQnnHtpV??Skel.so` set.
 *
 * `OrtEnvironment` and the session are lazily created on first call to [embed] — QNN graph
 * finalization at session-create takes ~16 s the first time (cached for subsequent app starts), so
 * we don't pay it at app launch.
 */
@Singleton
class EncoderRuntime @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    /**
     * Run the encoder on `batch` consecutive `WINDOW_SAMPLES`-sized FP32 windows packed into
     * [windowsFlat]. Returns a `(batch, EMBEDDING_DIM)` array of L2-normalized embeddings.
     *
     * The model has a baked-in batch=1 reshape op, so we call the session once per window rather
     * than resizing the input tensor.
     */
