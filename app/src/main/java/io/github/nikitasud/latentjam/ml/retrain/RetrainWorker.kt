...
/**
 * v1 stub for the on-device fine-tune pass. The data plumbing is fully in place — we know
 * how many events we have, what their schema is, and we hold a Room DAO that the next PR
 * can iterate over to build TrainPair tuples. This worker writes a Timber marker so we can
 * verify the trigger fires under realistic conditions without standing up the ORT Training
 * artifacts yet.
 */
@HiltWorker
class RetrainWorker
...
constructor(
    ...
    companion object {
        ...
        fun enqueue(context: Context, eventCount: Int) {
            val request = OneTimeWorkRequestBuilder<RetrainWorker>()
                .setInputData(
                    Data.Builder().putInt(KEY_TRIGGER_COUNT, eventCount).build()
                )
                .setConstraints(
                    Constraints.Builder()
