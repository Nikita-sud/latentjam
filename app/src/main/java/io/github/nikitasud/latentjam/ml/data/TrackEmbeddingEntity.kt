 ...
 * `modelVersion` lets us re-embed everything when the encoder ships a new checkpoint or
 * when the feature bundle gains a field — bumping the version forces a full backfill.
 *
 * `tempo` (BPM) is nullable because the autocorrelation estimator declines to commit on
 * non-percussive material rather than producing confident garbage. `energy` is
 * deterministic so it's always present once a row exists for this version.
 */
