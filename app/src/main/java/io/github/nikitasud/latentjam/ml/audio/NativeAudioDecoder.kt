 ...
 * NativeAudioDecoder.kt is part of LatentJam.
 *
 ...
 * 3. Seeks + decodes exactly `durationUs` of audio starting at `startUs`. Output is downmixed to
 *    mono float32 PCM at the source sample rate.
 *
 * Why we bother: `android.media.MediaExtractor.seekTo` charges 3-15 s/track for random offsets
 * across all of the formats above. The native path is 30-150 ms regardless of file size or offset.
 *
 * Anything we can't identify, plus failures, fall through to `MediaExtractor` in `AudioDecoder`.
 * The lib is loaded lazily; if the .so isn't on the device (unusual ABI, stripped from APK, etc.)
 * [tryDecode] returns null and everything still works.
 */
internal object NativeAudioDecoder {
    ...
    /**
     * Try to decode a mono float32 slice of [uri] starting at [startUs] for [durationUs]. Returns
     * null if the file isn't an mp3/opus, the native lib isn't available, the AssetFileDescriptor
     * has a nonzero start offset (we don't handle multi-file bundles yet), or the native decoder
     * hits any error. The caller is expected to fall back to the platform `MediaExtractor` path in
     * that case.
     */
    fun tryDecode(
        context: Context,
    ...
    /**
     * Cheap sniff of the first few bytes to predict whether the native path will succeed. Returns
     * true for files we'd route to native (mp3 or Ogg). Useful when callers want to avoid even
     * opening the AFD for known-bad formats. Currently unused (we let the native side sniff inside
     * [tryDecode]), but kept for callers that want to gate by format before paying for the AFD
     * open.
     */
    ...
    @JvmStatic private external fun nativeSniff(header: ByteArray): Int
    ...
    data class NativeDecodeResult(
        ...
        override fun equals(other: Any?): Boolean =
            other is NativeDecodeResult &&
                samples.contentEquals(other.samples) &&
                sourceSampleRate == other.sourceSampleRate

        override fun hashCode(): Int = samples.contentHashCode() * 31 + sourceSampleRate
    }
}
