 ...
 * EmbeddingWorker.kt is part of LatentJam.
 *
...
/**
 * Background pass that embeds every song in the device library that doesn't yet have a cached
 * embedding under the current model version. Foreground service so the OS lets the pass survive
 * screen-off; constrained to non-low-battery.
 */
@HiltWorker
...
constructor(
        ...
        withContext(Dispatchers.IO) {
            ...
            for ((idx, song) in pending.withIndex()) {
                ...
                if (
                    ...
                    foregroundOk =
                        trySetForeground(makeForegroundInfo(idx + 1, pending.size, startedAtMs))
                }
            }
     ...
     * `ForegroundServiceStartNotAllowedException` (Android 14+ background-start restriction) —
     * caller can keep working without a notification.
     */
    private suspend fun trySetForeground(info: ForegroundInfo): Boolean =
        try {
            setForeground(info)
            true
        } catch (e: Throwable) {
     ...
     * - elapsed is measured from the start of *this* worker run (not from the first library scan),
     *   so retries restart the timer cleanly.
     * - ETA is `(total - progress) * avg-ms-per-track` rounded to seconds; displays "—" while we
     *   don't have any successful embeds to base the estimate on.
     */
    private fun makeForegroundInfo(progress: Int, total: Int, startedAtMs: Long): ForegroundInfo {
        ...
        val etaMs =
            if (progress > 0 && progress < total) (total - progress).toLong() * msPerTrack else 0L
