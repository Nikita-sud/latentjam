...
/**
 * Tracks the user's current listening session and exposes the session features the
 * recommender's state encoder consumes. Mirrors the recipe in
 ...
 *     // NB: scoring_v1 was trained with session_feat_dim=4 (no mean_played_pct)
 *   ]
 *
 * The 5th field (`mean_played_pct`) is included in the public API so newer predictor
 * checkpoints can opt in without refactoring the recorder. The `PredictorRuntime`
 * truncates to whatever `session_feat_dim` the loaded ONNX expects.
 *
...
class SessionTracker @Inject constructor() {
    ...
    /**
     * Newest-first ring buffer of the last K=4 completed plays in the current session,
     * mirroring what `history_small` needs. Populated by the recorder on every finalize
     * regardless of the privacy opt-in, so Smart shuffle can run without the user enabling
     * persistent event logging.
     */
    private val recentCompleted: ArrayDeque<CompletedPlay> = ArrayDeque()

    /** Called when a new track index becomes the current one. */
    fun onTrackStart(nowMs: Long): TrackContext = synchronized(lock) {
    ...
    fun onTrackFinalized(event: ListeningEventEntity): Unit = synchronized(lock) {
        sessionPos += 1
        lastEventEndedAtMs = event.endedAtMs
        lastEventWasSkipped = event.skipped
        val playedPct = event.trackDurationMs.takeIf { it > 0 }
    ...
    fun recentCompletedSnapshot(): List<CompletedPlay> = synchronized(lock) {
        recentCompleted.toList()
    }

    /** Called when playback stops entirely. Does NOT advance `sessionPos`. */
    fun onSessionEnded(): Unit = synchronized(lock) {
    ...
    fun snapshot(nowMs: Long): SessionSnapshot = synchronized(lock) {
        val minutesSinceStart = if (sessionStartedAtMs == 0L) {
            0.0
        } else {
            max(0L, nowMs - sessionStartedAtMs) / 60_000.0
        }
        val completionRate = if (recentResults.isEmpty()) {
            0f
        } else {
            recentResults.count { it.completed }.toFloat() / recentResults.size
        }
        val meanPlayedPct = if (recentResults.isEmpty()) {
            0f
        } else {
        ...
        SessionSnapshot(
            ...
            completedThisSession = recentResults.count { it.completed }.let {
                // Account for completed plays that have already aged out of the recent window:
                // session_pos is incremented for every finalized event, recent_results is
                // capped at RECENT_WINDOW. Use sessionPos as a lower bound when the window
