...
/**
 * Lightweight bottom dialog for editing a song's metadata overrides. The user can set
 * genre / artist / year; the values layer over the audio file's tags via
 * [TrackMetadataResolver] and feed [io.github.nikitasud.latentjam.ml.predictor.MetadataRerank].
 *
...
class EditMetadataDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val uidString = arguments?.getString(ARG_SONG_UID)
            ?: error("EditMetadataDialog requires ARG_SONG_UID")
        val songUid = Music.UID.fromString(uidString)
            ?: error("Invalid Music.UID: $uidString")
        val song = musicRepository.library?.findSong(songUid)
            ?: error("Song not in library: $uidString")
        ...
        genreField.setAdapter(
            ArrayAdapter(ctx, android.R.layout.simple_list_item_1, genres)
        )
                ...
                lifecycleScope.launch {
                    try {
                        ...
                        Timber.i(
                            ...
                            songUid, newGenre, newArtist, newYear,
                        )
