 ...
 * `ShuffleMode.SMART`; this interface covers the privacy-sensitive bit (event logging) and
 * runtime parameters that don't fit on the playback UI.
 */
interface MlSettings : Settings<MlSettings.Listener> {
    ...
    /**
     * Encoder version string. Bumping this re-embeds every track on next library scan,
     * so we keep it scoped to anything that affects the embedding distribution:
     * encoder weights, mel-spec params, audio decode + resampler. The predictor
     * version is tracked separately via [predictorVersion] so swapping in a fresh
     * scorer doesn't trigger an unnecessary full library re-embed.
     *
    ...
    /**
     * Predictor (state encoder + scorer) version string. Bumping this hot-swaps
     * the ONNX assets but leaves the embedding store intact. Sourced from
     * `assets/ml/predictor_version.txt`.
     */
    val predictorVersion: String

    /**
     * `<embeddingVersion>+<predictorVersion>`. Diagnostic-only: written to JSONL
     * logs and shown in settings so we can correlate a sweep file with the
     * exact (encoder, predictor) pair that produced it.
     */
